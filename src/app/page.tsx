...
const Home = async (): Promise<JSX.Element> =>
  Effect.runPromise(
    pipe(
      Effect.all([
        getHighlights()
          .pipe(Effect.map((highlights) => highlights.slice(0, 3)))
          .pipe(
            Effect.map(Either.fromNullable(() => new Error('no highlights found'))),
            Effect.flatMap(
              Either.map((highs) => (
                <section key="highlights">
                  <Highlights title="Highlights" highlights={highs} />
                </section>
              )),
            ),
          ),
          ...
          Effect.flatMap(
            Either.map((cats) => (
              <section key="categories">
                <Categories uri={withBaseUrl('/categories')} title="Categories" categories={cats} />
              </section>
            )),
          ),
        ),
        getReviewedPreprints().pipe(
          ...
          Effect.flatMap(
            Either.map((rps) => (
              <section key="reviewed-preprints">
                <Teasers
                  uri={withBaseUrl('/reviewed-preprints')}
                  title="Recent Reviewed Preprints"
                  teasers={rps.slice(0, 10)}
                />
              </section>
            )),
          ),
