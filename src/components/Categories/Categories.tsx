      ...
      <ul className="category-tags">
        {pipe(
          ...
          Array.map((category, i) => (
            <li key={i} className="category-tags_item">
              <a href={withBaseUrl(`/categories/${category.id}`)} className="category-tags_item_link">
                {category.name}
              </a>
            </li>
          )),
...
export const Categories = ({ title, uri, categories }: CategoriesProps): JSX.Element => (
  <>
    {categories.length > 0 && (
      <section className="categories">
        <h2 className="categories__title">
          {uri ? (
            <a href={uri} className="categories__title_link">
              {title}
            </a>
          ) : (
            title
        ...
        <ul className="categories__list">
          {categories.map((category, i) => (
            <li key={i} className="categories__list_item">
              <div className="category">
                ...
                <div className="category__content">
                  <h2 className="category__title">
                    <a href={category.uri} className="category__title_link">
                      {category.name}
                    </a>
                  </h2>
                  <p
