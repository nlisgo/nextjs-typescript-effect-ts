...
const getHighlightsTopUpPage = () => getItemsTopUpPage(apiBasePath, highlightCodec, true);

const highlightsTopUpWrite = () =>
  pipe(
    getHighlightsTopUpPage(),
    Effect.tap((after) => Effect.log(`Total after: ${after.length}`)),
    Effect.map(stringifyJson),
    Effect.tap((highlights) =>
      Effect.flatMap(FileSystem.FileSystem, (fs) => fs.writeFileString(getCachedListFile, highlights)),
    ),
  );
...
export const highlightsTopUp = (): Effect.Effect<void, never, FileSystem.FileSystem | HttpClient.HttpClient> =>
  pipe(createCacheFolder(), Effect.flatMap(highlightsTopUpWrite), Effect.catchAllCause(Effect.logError), Effect.asVoid);
  ...
  pipe(
    ...
    Effect.map(
      Array.map((highlight) => ({
        ...
        image: {
          uri: iiifUri(highlight.image, imageWidth ?? 463, imageHeight ?? 260),
