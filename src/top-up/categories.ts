...
import { categoriesCodec, categoryCodec } from '@/codecs';
...
const retrieveIndividualCategories = (categories: Array<{ id: string; path: string }>) =>
  Effect.forEach(categories, retrieveIndividualCategory, {
    // this is the default, but you can be explicit:
    concurrency: 'unbounded',
  });

const categoriesTopUpPath = ({
  limit = 10,
  page = 1,
}: {
  limit?: number;
...
const getCategoriesTopUpPage = ({ limit, page = 1 }: { limit: number; page?: number }) =>
  getItemsTopUpPage(categoriesTopUpPath({ limit, page }), categoryCodec);
...
const missingIndividualCategories = pipe(
  ...
  Effect.flatMap((paths) =>
    Effect.flatMap(FileSystem.FileSystem, (fs) =>
      Effect.all(
        paths.map((id) =>
          fs
            .exists(`${getCachedFilePath}/${id}.json`)
  ...
  Effect.map((results) => results.filter(({ exists }) => !exists).map(({ id, path }) => ({ id, path }))),
);

const retrieveMissingIndividualCategories = () =>
  pipe(missingIndividualCategories, Effect.flatMap(retrieveIndividualCategories));

const getCategoriesTopUp = ({ limit, offset = 0 }: { limit: number; offset?: number }) =>
  pipe(
...
const categoriesTopUpWrite = ({ limit, total }: { limit: number; total?: number }) =>
  pipe(
    getCachedCategories(),
    ...
    Effect.tap((categories) =>
      Effect.flatMap(FileSystem.FileSystem, (fs) => fs.writeFileString(getCachedListFileNew, categories)),
    ),
  );
...
const categoriesTopUpCombine = () =>
  pipe(
    ...
    Effect.map(Array.sort(Order.mapInput(Order.string, (item: CategorySnippet) => item.id))),
    Effect.tap((after) => Effect.log(`Total after: ${after.length}`)),
    Effect.map((categories) => stringifyJson(categories)),
    Effect.tap((categories) =>
      Effect.flatMap(FileSystem.FileSystem, (fs) => fs.writeFileString(getCachedListFile, categories)),
    ),
  );

const categoriesTopUpInvalidate = () =>
  pipe(
...
const prepareCategorySnippet =
  ({ image, imageWidth, imageHeight }: { image: Image; imageWidth?: number; imageHeight?: number }) =>
  (category: Omit<CategorySnippet, 'image'>): CategoryProps => ({
    id: category.id,
    name: category.name,
    uri: withBaseUrl(`/categories/${category.id}`),
    image: {
      uri: iiifUri(image, imageWidth ?? 100, imageHeight ?? 100),
      alt: image.alt,
...
export const getCategory = ({
  ...
  imageWidth?: number;
  imageHeight?: number;
}): Effect.Effect<CategoryProps, PlatformError.PlatformError | ParseResult.ParseError, FileSystem.FileSystem> =>
  pipe(
