...
export const categoriesPath = ({
  ...
  limit?: number;
  page?: number;
} = {}): string => `${apiBasePath}/subjects?order=asc&page=${page}&per-page=${Math.min(limit, 100)}`;
...
export const reviewedPreprintsPath = ({
  ...
  `${apiBasePath}/search?sort=date&order=desc&type[]=reviewed-preprint&page=${page}&per-page=${Math.min(limit, 100)}`;
